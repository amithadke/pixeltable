...
class RemoteTable:
    ...
    def as_dict(self) -> dict:
        """Serialize RemoteTable to wire-format dict (remote_table_path)."""
        return {'remote_table_path': self._remote_path}

    @classmethod
    def from_dict(cls, d: dict) -> 'RemoteTable':
        """Deserialize RemoteTable from wire-format or legacy dict."""
        path = d.get('remote_table_path') or d.get('path')
        if path is None:
            raise KeyError('RemoteTable dict must have "remote_table_path" or "path"')
        return cls(path=path, remote_metadata=d.get('remote_metadata'))
...
class RemoteDir:
    ...
    def as_dict(self) -> dict:
        """Serialize RemoteDir to wire-format dict (remote_dir_path)."""
        return {'remote_dir_path': self._remote_path}

    @classmethod
    def from_dict(cls, d: dict) -> 'RemoteDir':
        """Deserialize RemoteDir from wire-format or legacy dict."""
        path = d.get('remote_dir_path') or d.get('path')
        if path is None:
            raise KeyError('RemoteDir dict must have "remote_dir_path" or "path"')
