...
from pixeltable.func import function as func_module
...
def _(obj: RemoteTable) -> dict[str, str]:
    """Serialize RemoteTable using as_dict() (wire format)."""
    return obj.as_dict()


@serialize_to_remote.register
def _(obj: RemoteDir) -> dict[str, str]:
    """Serialize RemoteDir using as_dict() (wire format)."""
    return obj.as_dict()
...
def _deserialize_table(value: dict[str, str]) -> Any:
    """Deserialize Table objects using RemoteTable.from_dict()."""
    if isinstance(value, dict) and ('remote_table_path' in value or 'path' in value):
        return RemoteTable.from_dict(value)
    return value


def _deserialize_remote_dir(value: dict[str, str]) -> Any:
    """Deserialize Dir objects using RemoteDir.from_dict()."""
    if isinstance(value, dict) and ('remote_dir_path' in value or 'path' in value):
        return RemoteDir.from_dict(value)
    return value
...
def _deserialize_table_or_dataframe(value: Any) -> Any:
    ...
    elif isinstance(value, dict) and ('remote_table_path' in value or 'path' in value):
        return RemoteTable.from_dict(value)
...
def _serializer_deserializer_from_as_dict_from_dict(
    cls: type,
) -> tuple[Callable[[Any], Any], Callable[[Any], Any]]:
    """
    Return (serialize_fn, deserialize_fn) using cls.as_dict and cls.from_dict.

    Any Pixeltable class used in remote ser/des must implement as_dict() and from_dict().
    """
    if not hasattr(cls, 'as_dict'):
        raise TypeError(
            f'{cls.__name__} must implement as_dict() for remote serialization; '
            'any Pixeltable class in ser/des must have as_dict() and from_dict().'
        )
    if not hasattr(cls, 'from_dict'):
        raise TypeError(
            f'{cls.__name__} must implement from_dict() for remote deserialization; '
            'any Pixeltable class in ser/des must have as_dict() and from_dict().'
        )
...
def _build_pixeltable_type_registry() -> dict[Any, tuple[Callable[[Any], Any], Callable[[Any], Any]]]:
    ...
    Pixeltable classes used in remote ser/des must implement as_dict() and from_dict().
    """
    ...
    # RemoteTable and RemoteDir use as_dict/from_dict (required for Pixeltable classes in ser/des)
    registry[RemoteTable] = _serializer_deserializer_from_as_dict_from_dict(RemoteTable)
    registry[RemoteDir] = _serializer_deserializer_from_as_dict_from_dict(RemoteDir)
    # Function (UDF) e.g. when passed in schema or other args to @remote APIs
    registry[func_module.Function] = _serializer_deserializer_from_as_dict_from_dict(func_module.Function)
    ...
    # Add Union type handling for Table | DataFrame (Pixeltable's DataFrame, not pandas) if available
    try:
        ...
        pass  # pixeltable.dataframe may not exist (e.g. Query in _query.py only)
...
def _create_pixeltable_customized_field(field_name: str, field_type: Any) -> tuple[Any, Any]:
    ...
    Priority:
    1. If type has as_dict() and from_dict() -> use them (required for Pixeltable classes in ser/des)
    ...
    # Use as_dict/from_dict when present (no Pydantic mixin)
    if hasattr(field_type, 'as_dict') and hasattr(field_type, 'from_dict'):
        try:
            serializer_func, validator_func = _serializer_deserializer_from_as_dict_from_dict(field_type)
        except TypeError:
            pass
        else:
